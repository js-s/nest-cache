...
NestCash — personal budget tracking web app. Single-user MVP, API-first backend in Go serving JSON. No frontend code lives in this repo (separate deployable).

## Current state

**Pre-scaffold.** Only `go.mod` and planning docs exist. No source code, tests, CI, or Dockerfile yet. Do not search for existing handlers, models, or routes — they don't exist.

## Locked decisions (from @context/foundation/tech-stack.md and @context/foundation/prd.md)

- **Language:** Go 1.22.2, standard library preferred over frameworks
- **Auth:** email + password with long-lived sessions; assembled from Go ecosystem libs (not a full framework)
- **Deployment:** self-hosted, single compiled binary
- **CI:** GitHub Actions, auto-deploy-on-merge
- **Scope:** single-user; no multi-user, no charts, no offline, no push notifications in MVP

## Architecture constraints

- API-first: backend exposes JSON endpoints; clients are separate
- Predefined categories seeded on first use
- Budget ratio warning when expenses/income >= 80% (passive UI indicator, not blocking)
- Data privacy: one user's data must never leak to another context

## Canonical sources of truth

| What | Where |
|------|-------|
| Full requirements & user stories | `@context/foundation/prd.md` |
| Stack rationale & deployment | `@context/foundation/tech-stack.md` |
| Original brainstorm (Polish) | `@idea-notes.md` |
| Lessons learned (append-only) | `@context/foundation/lessons.md` |

Read `prd.md` before implementing any feature — it contains acceptance criteria and explicit scope cuts.

## Conventions (to establish during scaffold)

When scaffolding, follow standard Go project layout:
- `cmd/nest-cash/` — main entrypoint
- `internal/` — application code
- Flat package structure: one package per domain concern inside internal/. Only split into sub-packages when a package exceeds ~500 LOC or has distinct public API boundaries.

Lint with `golangci-lint` once configured.

## OpenCode permissions

Defined in `@context/opencode.json`. Notable: `git push` requires confirmation; `rm -rf` is denied.
