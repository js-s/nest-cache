---
bootstrapped_at: 2026-09-10T10:09:50Z
starter_id: go
...
**Exit code**: 0
**Files moved**: 1 (`go.mod` → `go.mod.scaffold`)
...
**Reason**: `govulncheck` is not installed (`command not found`)
...
The Go starter intentionally provides only a module. The deployment preparation adds the minimal HTTP entrypoint and Docker/Fly configuration needed for a smoke deploy; NestCash business features remain separate implementation work.
