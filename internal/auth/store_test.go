...
func TestStore(t *testing.T) {
	...
	if _, ok, err := s.CreateUser(ctx, NormalizeEmail(email), "hash-2"); err != nil || ok {
		t.Fatalf("duplicate CreateUser = ok %v, err %v", ok, err)
	}

	found, ok, err := s.FindUserByEmail(ctx, NormalizeEmail(email))
	...
	if err := s.CreateSession(ctx, live, user.ID, future); err != nil {
